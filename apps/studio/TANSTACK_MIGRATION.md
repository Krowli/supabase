...
- [x] `routes/api/platform/auth/$ref/**` — the five fork routes added by `fork/auth-selfhosted` (`config`, `config/hooks`, `templates/$template/content`, `templates/$template/reset`, `validate/spam`) are mirrored and registered in `routeTree.gen.ts`. The tree was regenerated programmatically — `Generator` + `getConfig` from `@tanstack/router-generator`, given `routeTreeFileFooter` from `@tanstack/start-plugin-core`'s `buildRouteTreeFileFooterFromConfig`; without that footer the run deletes the `declare module '@tanstack/react-start'` Register block. Full script and procedure in the fork's local (untracked) `.superpowers/sdd/studio-fork-auth/task-3-report.md`.
