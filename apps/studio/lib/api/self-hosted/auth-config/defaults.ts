 ...
 *
 * Every value is GoTrue's own default, taken from the `default:` struct tags and the
 * `ApplyDefaults` fallbacks in `internal/conf/configuration.go`. A field with no GoTrue default
 * gets the zero value GoTrue itself would run with: `false` for booleans, `0` for numbers that
 * GoTrue reads as "unset". Values that have no meaningful zero are listed in `NULL_BY_DEFAULT`
 * instead.
 *
 * `defaults.test.ts` parses `GoTrueConfigResponse` out of `platform.d.ts` and fails if a key is
 * missing from both lists or present in both, so this stays complete as the type moves.
 */
export const DEFAULTS: Partial<GoTrueConfigResponse> = {
...
/**
 * Keys that are `null` when unset rather than carrying a zero value: secrets, hostnames, URLs and
 * templates with no GoTrue default, plus the session durations that mean "no limit" when absent
 * (`*time.Duration` in Go, so `nil` rather than `0`).
 */
export const NULL_BY_DEFAULT: readonly (keyof GoTrueConfigResponse)[] = [
