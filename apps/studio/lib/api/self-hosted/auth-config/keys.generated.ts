/**
 * The platform auth-config contract: every key, with the JavaScript type its value must have.
 *
 * The union of `GoTrueConfigResponse` (`packages/api-types/types/platform.d.ts:9244-9512`) and
 * `UpdateGoTrueConfigBody` (`packages/api-types/types/platform.d.ts:13542-13796`), sorted. Where
 * the two blocks declare a key differently, the update body wins, because this table is what
 * `validatePatch` checks an incoming PATCH against. Three keys differ that way, each a plain
 * `string` in the response and a fixed set of values in the update body:
 * `PASSWORD_REQUIRED_CHARACTERS`, `SECURITY_CAPTCHA_PROVIDER` and `SMS_PROVIDER`.
 *
 * Generated, not hand-written. `keys.generated.test.ts` re-extracts both blocks from
 * `platform.d.ts` and fails when this table drifts — a key added, a key removed, or a type changed.
 *
 * The two blocks also disagree about which keys exist: `EXTERNAL_WORKOS_ENABLED` and the four
 * `EXTERNAL_X_*` keys are writable but never returned, and `CUSTOM_OAUTH_MAX_PROVIDERS` plus the
 * two `*_CUSTOM_CONTENTS` keys are returned but not writable. Studio has to store all of them, so
 * the union is the list rather than either block.
 */

/** A declared value type: a `typeof` name, `'object'`, or the exact values a key accepts. */
export type PlatformValueType = 'string' | 'number' | 'boolean' | 'object' | readonly string[]

export const PLATFORM_CONFIG_TYPES: Readonly<Record<string, PlatformValueType>> = Object.freeze({
  ...
  DB_MAX_POOL_SIZE_UNIT: ['connections', 'percent'],
  ...
  MAILER_SUBJECTS_CUSTOM_CONTENTS: 'object',
  ...
  MAILER_TEMPLATES_CUSTOM_CONTENTS: 'object',
